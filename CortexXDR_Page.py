#!/usr/bin/env python3
from CortexXDR_Alerts import CortexAlerts, CortexTable
from nicegui import ui
import json

class CortexPage:
    def __init__(self) -> None:
        ui.label('Cortex XDR').classes('text-2xl')
        with ui.row().classes("flex flex-nowrap w-full h-full"):
            with ui.column().classes("w-1/3"):
                self.BASE_URL = ui.input("Base URL").classes("w-full pr-10")       
                self.APP_ID = ui.input("APP ID").classes("w-full pr-10")   
        ...
        def get_alerts():
            try:
                cortex_alerts = CortexAlerts(self.BASE_URL.value, self.APP_ID.value, self.API_SECRET.value, self.DATE_RANGE.value) 
                if cortex_alerts.getAlerts().status_code == 200:
                    data = cortex_alerts.getAlerts().json()
                    content = CortexTable(data).table()
                    self.tabled_alerts.set_content(f"<pre>{content}</pre>")
                    self.TABLED_RESULTS.set_visibility(True)
                    return json.dumps(data, indent=4) 
        
            except Exception as e:
                if self.BASE_URL.value == '' or self.APP_ID.value == '' or self.API_SECRET.value == '' or self.DATE_RANGE.value =='':
                    ui.notify("Error. Some or all API Credential is empty",  position="top-right", type="warning")
                elif cortex_alerts.getAlerts().status_code != 200:
                    return cortex_alerts.getAlerts().text
                else:
